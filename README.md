...
*Pick a job, get an explainable fit score, skill gaps, and generated interview questions, then ask grounded, cited follow-ups.*
...
3. **Select a job** to see an **explainable fit score**, matched vs. missing skills, a written
   fit summary (addressed to the candidate), and **interview questions** generated for that role.
4. **Chat** to ask anything ("What am I missing for this role?", "How does my experience align?").
...
(one JD, many résumés); that's noted under [what I'd do differently](#what-id-do-differently-with-more-time).
...
stores both the relational data and the embeddings in one datastore, with no second system.
...
- Retrieved chunks are formatted with explicit `[resume §section]` / `[JD §section]` tags. The system
  ...
  UI over SSE; the frontend renders answers as Markdown (tables, lists) with the citation tags as chips.
...
- **Retrieval-recall eval** (`tests/test_eval.py`): a fixture of question/expected-section cases
  asserts retrieval recall of at least 0.8.
- **LLM-as-judge groundedness gate** (`evals/judge.py`, `tests/test_llm_judge.py`): a separate,
  cheap Claude call scores each chat answer's faithfulness to the retrieved context on a 1 to 5
  rubric; the suite fails if the average drops below threshold. This runs in CI when secrets are
  present and auto-skips locally when keys are absent.
...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/resumes` | Upload a résumé PDF, then parse, chunk, embed |
| `GET`  | `/resumes/latest` | Most recently uploaded résumé (powers auto-load) |
| `POST` | `/jobs` | Add a job, then extract structured JD, chunk, embed |
...
│   ├── components/               # ResumeUpload, JobList, FitDashboard, ChatPanel, etc.
...
| Decision | Choice | Why |
|---|---|---|
| **Chunking** | Section-aware for résumés; recursive windows for JDs | Résumés are short and structured, so section chunks keep bullets whole and enable section-filtered retrieval. JDs are prose, so a sliding window fits. |
...
| **Quality** | Deterministic score + retrieval-recall eval + LLM-judge gate | Score is computed and explainable; answers are scored for faithfulness, gated in CI. |
...
> Rewrite this in your own voice before submitting; the draft below is a starting point.

I treated the AI assistant as a fast pair-programmer, not an autopilot. The workflow was
spec, then plan, then small test-backed increments, and I reviewed every diff before it landed.
...
high-volume, low-judgment, and easy to verify by running them, which is exactly where AI earns its keep.

**Where I deliberately didn't trust it and decided myself:** the parts that need domain judgment and
that I'd have to defend later, namely the section-aware chunking strategy, the deterministic scoring
formula and its weights, and the prompt contracts (the citation-tag format, the JD-extraction JSON
shape). I had AI draft, but I owned the decision and the wording. When a model proposed adding
...
- Gap to learning-plan generator.
